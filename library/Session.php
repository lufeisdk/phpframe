...
class Session
{
    ...
    public static function getInstance()
    {
        static $instance;
        if (!$instance) {
            $config = Config::all(BIND_MODULE, 'session');
    ...
    public function init(array $config = [])
    {
        ...
        /*if (!empty($config['type'])) {
            ...
            $class = false !== strpos($config['type'], '\\') ? $config['type'] : '\\phpframe\\session\\driver\\' . ucwords($config['type']);

            // 检查驱动类
            if (!class_exists($class) || !session_set_save_handler(new $class($config))) {
                throw new NotFoundException('error session handler:' . $class, $class);
            }
        }*/
